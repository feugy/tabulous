...
describe('RuleViewer component', () => {
  ...
  it('displays first page', async () => {
    ...
    expect(image).toHaveAttribute('src', `/images/${game}/rules/1.png`)
  ...
  it('navigates upward up to the last page', async () => {
    ...
    await tick()
    expect(image).toHaveAttribute('src', `/images/${game}/rules/2.png`)
    ...
    fireEvent.click(next)
    await tick()
    expect(image).toHaveAttribute('src', `/images/${game}/rules/3.png`)
    expect(previous).toBeEnabled()
