...
/**
 * @typedef {object} Options authentication plugin options, including:
 ...
 * @property {Omit<import('../services/auth/oauth2').ProviderInit, 'redirect'>} [github] - Github authentication provider options.
 * @property {Omit<import('../services/auth/oauth2').ProviderInit, 'redirect'>} [google] - Google authentication provider options.
 */
...
export default async function registerAuth(app, options) {
  ...
  for (const provider of [githubAuth, googleAuth]) {
    ...
    if (name in options) {
      provider.init({
        .../** @type {Omit<import('../services/auth/oauth2').ProviderInit, 'redirect'>}} */ (
          options[name]
        ),
